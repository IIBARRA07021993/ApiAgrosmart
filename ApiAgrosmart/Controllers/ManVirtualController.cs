...
namespace ApiAgrosmart.Controllers
{
    ...
    public class ManVirtualController : ApiController
    {
        ...
        [HttpPut]
        [Route("api/ManifiestoVirtual")]
        public string ManifiestoVirtual(string as_empresa, int as_operation, string as_json)
        {
            WebLog.Write(EventLogType.Information, "Iniciando  Operacion =" + as_operation + "[ManifiestoVirtual]");
            var sp = "sp_AppManifiestoVirtual";
            int success = 0;
            string message = "";
            ...
            using (var sqlCon = new SqlConnection(configuration.Conexion))
            {
                try
                {
                    WebLog.Write(EventLogType.Information, "Abriendo Conexion ala Base de datos[ManifiestoVirtual]");
                    sqlCon.Open();
                    ...
                    WebLog.Write(EventLogType.Information, "Ejecutando Procedimiento sp_AppManifiestoVirtual [ManifiestoVirtual]");
                ...
                {
                    WebLog.Write(EventLogType.Error, "Cerrando Conexion finally [ManifiestoVirtual]");
                    sqlCon.Close();
                }
                WebLog.Write(EventLogType.Information, "Terminando Operacion Exitosa[ManifiestoVirtual]");
