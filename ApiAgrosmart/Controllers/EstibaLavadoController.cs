...
namespace ApiAgrosmart.Controllers
{
    ...
    public class EstibaLavadoController : ApiController
    {
        ...
        [HttpPut]
        [Route("api/ControlEstibaLavado")]
        public string ControlEstiba(string as_empresa, int as_operation, string as_json)
        {
            WebLog.Write(EventLogType.Information, "Iniciando  Operacion =" + as_operation + "[ControlEstibaLavado]");
            var sp = "sp_AppControlEstibaLavado";
            int success = 0;
            string message = "";
            ...
            using (var sqlCon = new SqlConnection(configuration.Conexion))
            {
                try
                {
                    WebLog.Write(EventLogType.Information, "Abriendo Conexion ala Base de datos[ControlEstibaLavado]");
                    sqlCon.Open();
                    ...
                    WebLog.Write(EventLogType.Information, "Ejecutando Procedimiento sp_AppControlEstibaLavado [ControlEstibaLavado]");
                ...
                {
                    WebLog.Write(EventLogType.Error, "Cerrando Conexion finally [ControlEstibaLavado]");
                    sqlCon.Close();
                }
                WebLog.Write(EventLogType.Information, "Terminando Operacion Exitosa[ControlEstibaLavado]");
