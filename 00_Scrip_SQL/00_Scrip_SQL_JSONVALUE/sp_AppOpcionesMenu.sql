...
BEGIN
    BEGIN TRY
        ...
        SELECT @as_usuario = JSON_VALUE(@as_json, '$.c_codigo_usu'),
               @as_sis = JSON_VALUE(@as_json, '$.c_codigo_sis');
...
BEGIN
    BEGIN TRY
        ...
        /*SACAMOS LOS DATOS  DEL JSON*/
        SELECT @as_usuario = RTRIM(LTRIM(ISNULL(JSON_VALUE(@as_json, '$.c_codigo_usu'), ''))),
               @as_sis = RTRIM(LTRIM(ISNULL(JSON_VALUE(@as_json, '$.c_codigo_sis'), ''))),
               @as_permiso = RTRIM(LTRIM(ISNULL(JSON_VALUE(@as_json, '$.c_codigo_sme'), '')));
