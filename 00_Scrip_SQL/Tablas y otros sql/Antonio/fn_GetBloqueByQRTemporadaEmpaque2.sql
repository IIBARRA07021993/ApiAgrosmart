...
(
    SELECT *
    FROM dbo.sysobjects
    WHERE id = OBJECT_ID(N'[dbo].[fn_GetBloqueByQRTemporadaEmpaque2]')
)
    DROP PROCEDURE fn_GetBloqueByQRTemporadaEmpaque2;

GO
/*|AGS|*/
CREATE FUNCTION [dbo].[fn_GetBloqueByQRTemporadaEmpaque2]
(	
	@idQR				VARCHAR(22),
	@idTemporada		CHAR(2),
	@idEmpaque			CHAR(2)
...
RETURN 
(
	SELECT TOP 1 c_codigoext_lot 
	FROM dbo.t_lote (NOLOCK) 
	WHERE c_codigo_lot IN		(
